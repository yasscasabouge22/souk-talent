...
import { Search, Filter, MapPin, DollarSign, Calendar, Music, X } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface OrganizerAdvancedSearchDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSearch?: (filters: any) => void;
}

export function OrganizerAdvancedSearchDialog({
  isOpen,
  onClose,
  onSearch,
}: OrganizerAdvancedSearchDialogProps) {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-[#191414] border-gray-800">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-white flex items-center gap-2">
            <Search className="size-6 text-[#1DB954]" />
            Recherche Avancée d'Artistes
          </DialogTitle>
          <DialogDescription className="sr-only">
            Recherchez des artistes selon vos critères personnalisés
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 mt-4">
          ...
          <div className="bg-[#282828] rounded-lg p-6 border border-gray-800">
            <Label className="text-white font-semibold mb-3 block">Recherche par mot-clé</Label>
          ...
          <div className="bg-[#282828] rounded-lg p-6 border border-gray-800">
            <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
              <Filter className="size-5 text-[#1DB954]" />
              Filtres principaux
            </h3>
          ...
          {availableSubcategories.length > 0 && (
            <div className="space-y-3">
              <Label className="text-white font-semibold">Sous-catégories</Label>
          ...
          <div className="bg-[#282828] rounded-lg p-6 border border-gray-800">
            <Label className="text-white font-semibold mb-3 block">Options supplémentaires</Label>
            <div className="flex items-center gap-3">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                  verified
                    ? "bg-[#1DB954] text-black"
                    : "bg-[#191414] text-gray-400 border border-gray-700"
                }`}
              >
          ...
          <div className="bg-[#282828] rounded-lg p-6 border border-gray-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white font-semibold">Résultats estimés</p>
                <p className="text-gray-400 text-sm">Environ 47 artistes correspondent à vos critères</p>
              </div>
              <Badge className="bg-[#1DB954] text-black px-4 py-2 text-lg">
                47
              </Badge>
            </div>
          </div>
